...
float processarHierarquia(int index, int nivel_atual, int limite_profundidade) {
    ...
    float *ptmtotal = &drive[index].tamanho_total;
    *ptmtotal = drive[index].tamanho_proprio;
    ...
    for (int i = 0; i < drive[index].qtd_sub; i++) {
        ...
        int *pfilho_idx = &filho_idx;
        *ptmtotal += processarHierarquia(*pfilho_idx, nivel_atual + 1, limite_profundidade);
    }

    // Impressão com formatação de BI
    for (int i = 0; i < nivel_atual; i++) printf("  "); // Indentação
    
    printf("|-- %s [%.2f GB]", drive[index].nome, *ptmtotal);
...
int main() {
    ...
    processarHierarquia(0, 0, *plimite);
    printf("-------------------------------------------\n");
